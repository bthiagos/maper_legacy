$(document).ready(function() {
    ...
    if($("#organizacao").val() != "") {
        getGrupos($("#organizacao").val(),$("#linguagem").val());
    }
})
...
function getGrupos(id,lang) {
    var escolha = "(escolha)";
    var escolha_org = "(escolha uma organização)";
    var carregando = "(carregando grupos...)";
    if(lang == "en") {
        var escolha = "(choose)";
        var escolha_org = "(choose an organization)";
        var carregando = "(loading groups...)";
    }
    if(lang == "es") {
        var escolha = "(elegir)";
        var escolha_org = "(eligir una organización)";
        var carregando = "(cargando grupos...)";
    }
    ...
    $.ajax({ 
		...
		data: ({
	     ...
	     if(data != "fail") {
            ...
            $("#grupo").html('<option value="" selected="selected">'+escolha_org+'</option>');
...
function valida(formulario,campos,lang) {
	ok = 1;
	msg = "";
	...
	for(i = 0; i < campo.length; i++) {
        ...
        if(lang == "en") { campo_obrigatorio = "Required field!"; }
        if(lang == "es") { campo_obrigatorio = "Campo obligatorio!"; }
        ...
        if(campo_valor == "" && campo_id != "telefone") {
		    ...
		    if(campo[i] == "email") {
                ...
                if(!er.exec(campo_valor)) {
                    ...
                    if(lang == "en") { msg = "Invalid mail."; }
                    if(lang == "es") { msg = "Correo electrónico inválido."; } 
                    $(campo_objeto).css("border","1px solid #CD0000");
                    $(campo_objeto).after("<span id='erro_"+i+"' style='color: #CD0000;'> * "+msg+"</span>");
                }
            }
            ...
            if(campo[i] == "nascimento") {
                ...
                if(dia > 31 || mes > 12 || ano > (yy-10)) {
                    ...
                    if(lang == "en") { msg = "Invalid bith date."; }
                    if(lang == "es") { msg = "Fecha de nacimiento incorrecta."; }                    
                    $(campo_objeto).css("border","1px solid #CD0000");
                    $(campo_objeto).after("<span id='erro_"+i+"' style='color: #CD0000;'> * "+msg+"</span>");
                }
            }
            ...
            if(campo[i] == "ticket") {
                ...
                if($("#organizacao").val() == "" || $("#grupo").val() == "") {
                    ...
                    if(lang == "en") { msg = "Choose an Organization and Group."; }
                    if(lang == "es") { msg = "Elija una Organización y el Grupo"; } 
                    $(campo_objeto).css("border","1px solid #CD0000");
...
function valida_ticket(formulario,campos,lang) {
    ok = 1;
    msg = "";
    ...
    for(i = 0; i < campo.length; i++) {
        ...
        if(campo_valor == "" && campo_id != "telefone") {
            ...
            if(campo[i] == "ticket") {
                    $.ajax({
                        dataType: "html",
                        url: 'aplicacao_ajax/ticket2.php',
                        type: 'POST',
                        data: ({
                            lang:lang,
                            ticket:campo_valor
...
function respondeu(qual,tempo,resposta,ordem,protocolo,lang) {
	$("#slider_nav_"+qual).addClass("respondido");	
    $("#load").fadeIn('fast');
    
    var salvar_aplicacao = 0
    ...
    $.ajax({
		...
		data: ({
            ...
            ordem: ordem,
            protocolo: protocolo,
            salvar_aplicacao: salvar_aplicacao
		}),
       success: function(data) {
            if(data == 1) {            
...
function valida_protocolo(lang) {
    $("#protocolo").css("border","1px solid #CCC");
    $("#erro_protocolo").html("");
    
    if($("#protocolo").val() == "") {
        msg = "* Digite um protocolo.";
        if(lang == "en") { msg = "* Enter a protocol."; }
        if(lang == "es") { msg = "* Escriba un protocolo."; } 
        ...
        $.ajax({ 
    		...
    		success: function(data){
    	        if(data == "falha") {
                   ...
                   if(lang == "en") { msg = "* This protocol does not exist."; }
                   if(lang == "es") { msg = "* Este protocolo no existe."; } 
                    $("#protocolo").css("border","1px solid #CD0000");
                    $("#erro_protocolo").html(msg);
    	        } else {
