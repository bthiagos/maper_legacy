<? ob_start(); ini_set("allow_url_fopen", "On"); ini_set("display_errors",1) ?>
<? include("conn.php"); ?>

<?
$id_pesquisa = $_GET["id_pesquisa"];
$numero_participantes = mysql_query("SELECT id FROM pc_ticket WHERE id_pesquisa = '$id_pesquisa'");
...
while($pg = mysql_fetch_array($perg)) {
    ...
    if($tipo == 'Fechado') {
        ...
        while($alt = mysql_fetch_array($alts)) {
            ...
            $resp = mysql_query("SELECT id, aberta FROM pc_resposta WHERE id_alternativa = '$ida' and id_pesquisa = '$id_pesquisa' $filtro_alt $em_andamento") or die(mysql_error());
        ...
        $resp_aberta = mysql_query("SELECT aberta FROM pc_resposta WHERE id_pergunta = '$idp' and id_pesquisa = '$id_pesquisa' $filtro_alt $em_andamento");
