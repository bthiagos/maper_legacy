...
// Some data
$data = array($zerotres,$media,$seteoito,$nove,$dez);
...
$item = array();
$desc = mysql_query("SELECT * FROM feedbacks_grupo WHERE item = 'Intensidade'");
...
$p1->SetLegends(array("$zerotres%% ".$item[0],"$media%% ".$item[1],"$seteoito%% ".$item[2],"$nove%% ".$item[3],"$dez%% ".$item[4]));
$graph->legend->SetFont(FF_FONT2,FS_NORMAL,16);
$graph->legend->Pos(0.01,0.01);

$graph->Add($p1);
$graph->Stroke("graficos_grupos/intensidade.png");
?>