	...
	<div id="global">
		...
		<div id="principal">
			...
			<?php
                ...
                if (($_REQUEST["ano1"] != "") || ($_REQUEST["ano2"] != ""))  {
                    if($where == ""){
                        $where .= " WHERE YEAR(STR_TO_DATE(nasc, '%d/%m/%Y'))>='$ano1' AND YEAR(STR_TO_DATE(nasc, '%d/%m/%Y'))<='$ano2' ";
                    }else{
                        $where .= " and YEAR(STR_TO_DATE(nasc, '%d/%m/%Y'))>='$ano1' AND YEAR(STR_TO_DATE(nasc, '%d/%m/%Y'))<='$ano2' ";
                    }
                }
                ...
                if($where == "") {
                   $where .= " WHERE aplicacoes.aplicativo = 0";
                } else {
				   $where .= " and aplicacoes.aplicativo = 0";
				}

            ?>

			<form action="aplica_gerencia.php" method="POST" onsubmit="return valida_fromto();">
			...
			<div id="linha_form">
					<div id="label"> <span class="label_fonte">Organização:</span> </div>
						...
						if($organizacaon){
				          ...
				          //echo $result;
				          ?>
				          <select name="orga" id="orgs" style="width:200px;" onchange="get_grupos(<?=$_REQUEST["grupos"]?>)">
                ...
                <? } else { ?>
                <?
                $sql = "SELECT * FROM `grupos` WHERE id_organizacao = '$getorg1' ORDER BY `nome`";
				$result = mysql_query($sql);
                ?>
                <div id="linha_form">
					<div id="label"> <span class="label_fonte">Grupos:</span> </div>
                ...
                <div id="linha_form">
                    <div id="label"> <span class="label_fonte">Nascidos entre:</span> </div>

                    <span id="ano1">                   
                     <select name="ano1" id="ano1">
                        <option value="">Ano</option>
                        <? for ($i = date("Y"); $i > 1899; $i--) { ?>
                        <option value="<?=$i;?>" <? if($_REQUEST["ano1"] == $i) { echo "selected"; } ?>><?=$i;?></option>
                        <?}?>
                     </select>
                    </span>
                     <span class="label_fonte"> e </span>
                    <span id="ano2">                   
                     <select name="ano2" id="ano2">
                        <option value="">Ano</option>
                        <? for ($i = date("Y"); $i > 1899; $i--) { ?>
                        <option value="<?=$i;?>" <? if($_REQUEST["ano2"] == $i) { echo "selected"; } ?>><?=$i;?></option>
                        <?}?>
                     </select>
                    </span>
                </div>
                ...
                ?>
                <div id="linha_form">
					<div id="label"> <span class="label_fonte">Mostrar Pessoas:</span> </div>

                    <input type="checkbox" name="pessoas" value="1" <?php if($_REQUEST["pessoas"] == 1) { echo "checked"; } ?> />
                ...
                {
                ?>
                <div style="margin-left: 30px; font-family: Arial; font-size: 12px;"><a href="?ord=1">Ordenar por organização, grupo e pessoas</a></div>
                <? } else { ?>
                <div style="margin-left: 30px; font-family: Arial; font-size: 12px;"><a href="?">Ordenar por Últimos testes relizados</a></div>
                <? } ?>
                ...
                <div style="float: left; width: 80px;"><input type="submit" value="Gerar Extrato" class="form_style" name="extrato"></div>
...
limit 6;
            ...
            ?>
				...
				<table width="100%" align="center" class="sortable" cellspacing="3">
					<tr height="30">
                        ...
                        <td align="center">Tipo de Aplicação</td>
			...
			?>
                    ...
                    $check_ticket = mysql_query("SELECT operacional,tipo_ticket FROM gerador_tickets WHERE numero_ticket = '".$linha["ticket"]."'");
                    if(mysql_num_rows($check_ticket) != 0) {
                        $tipo_ticket_assoc = mysql_fetch_assoc($check_ticket);
                        ...
                        <div style="clear: both; margin-bottom: 10px;margin-top: 50px"><img src="imagens/enviar_rel.jpg" /></div>
                           ...
                           <div style="float: left; margin-left: 5px; margin-bottom: 15px;">
                               <select id="relatorio" name="relatorio">
                                   ...
                                   <? if($tipo_ticket == "gerencial" or (($permissao == "99991") or ($permissao == "99992"))) { ?>
                                       <option value="7">Gerencial - Português</option>
                                       <option value="8">Gerencial - Inglês</option>
                                       <option value="9">Gerencial - Espanhol</option>
                                       <option value="13">Gerencial sem nota</option>
                                       <option value="14">PDI</option>
                                   <? } ?>

                                   <? if($tipo_ticket == "operacional" or (($permissao == "99991") or ($permissao == "99992"))) { ?>
                                   <option value="10">Operacional</option>
                                   <? } ?>

                                   <? if($tipo_ticket == "vendas" or (($permissao == "99991") or ($permissao == "99992"))) { ?>
                                   <option value="11">Vendas</option>
                                   <? } ?>

                                   <? if($tipo_ticket == "vendas1" or (($permissao == "99991") or ($permissao == "99992"))) { ?>
                                   <option value="12">Vendas 1 página</option>
                                   <? } ?>
                                   <!--
                                   <option value="1">Relatório Base 10 com Gráficos</option>
                                   <option value="2">Relatório Base 10 sem Gráficos</option>
                                   <option value="4">Relatório Coaching com Gr?fico</option>
                                   <option value="5">Relatório Coaching sem Gr?fico</option>
                           ...
                           <div style="font-size: 12px; font-family: Arial, Verdana;">
                               ...
                               <div style="margin-top: 5px;"><input type="radio" name="sendhow" value="Individual" class="sendhow" /> Individual - <?=utf8_decode($linha["nome"]);?></div>
                           </div>
					...
					<tr height="30" class="cel_fonte" onMouseOver="this.className='cel_fonte_hover'" onMouseOut="this.className='cel_fonte'">
						<td align="center" ><?=$i;?> | <?=$linha["id"]?></td>
						<td align="center" ><?=$linha["databr"]?></td>
						<td align="left" ><?=utf8_decode($linha["nome"]); ?></td>
                        <td align="center" >
                            <?=$linha["nasc"];?> &nbsp;
                            <?

                                $idade = 0;
                                $data_nascimento = $linha["nasc"];
						...
						<? if($permissao != "99991") {?><td align="center" ><?=utf8_decode($linha["cargo"])?> </td><? } ?>
                        <td align="center" >
                        <style>
                            .inplace_save {
                                cursor: pointer;
                        ...
                        <td align="center" width="1%" nowrap>
                            ...
                            $mostra_gerencial = true;
                            $mostra_operacional = true;

                            if($tipo_ticket == "gerencial") { echo "Gerencial"; }
                            if($tipo_ticket == "operacional") { echo "Operacional"; }
