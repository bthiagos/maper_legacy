...
//	echo "<p><b>Compet̻ncias:</b></p>";
	while ($i<20){ 
		...
		<? echo $nota110 = base7(($i+1),$competencias[$i]); ?>
		</td><td align="left" valign="top" style="font-size:8px; font-align:center;"><div style="width: 100%">
...
function base7 ($pCompentencia, $pNota){

			$sql = " 
       		SELECT f.descricao, f.base7
			...
			return $a = mysql_result($sql, 0, "base7");
...
$notamedia2 = 0;
			$n = 0;
			while ($n<20){
				$nota110a = base7(($n+1),$competencias[$n]);
				$notamedia2 += $nota110a;
					$n++;
			}
	?>		
...
<p style="margin-top: 10px; margin-left: 25px;"><? include("tabela2_vendido_2.php"); ?></p>
<? 
$notamedia = 0;
			$i = 0;
			while ($i<20){
				$nota110 = base7(($i+1),$competencias[$i]);
				$notamedia += $nota110;
					$i++;
			}
	?>		
