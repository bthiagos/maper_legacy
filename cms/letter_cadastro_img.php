...
if ($_REQUEST['cadastra']) {
	...
	if (!$ok) {
		...
		alert("Algum campo foi preenchido incorretamente ou está em branco, tente novamente!");
		 ...
		 if(move_uploaded_file($arqTemp,"letter/img/".$arqName)) {
		 //CONECTA AO FTP
		
			$sql = "INSERT INTO ce_letter (titulo,tipo,arq) values ('$titulo',2,'$arqName')";
			// Confirmacao de insert

				if (mysql_query($sql)) {
					alert("Newsletter cadastrada com sucesso!");
					redireciona("letter_cadastro_img.php");
				} // IF QUERY
...
<br><br><br></div>
	...
	<div id="global">
		...
		<div id="principal">
			...
			<div id="info">
				<img src="imagens/cadastro_tipoImagem.gif" alt="Cadastro de Newsletter" />
			</div>
			<!-- INICIO - DIV info - Barra de informacao -->
			
			<form action="letter_cadastro_img.php?cadastra=1" method="post" name="cadastro" enctype="multipart/form-data">
			...
			<!-- INICIO - DIV info - Barra de informacao -->
			<div id="info">
				<img src="imagens/gerenciamento_imagem.gif" alt="Gerenciamento de Newsletters" />
			</div>
			<!-- INICIO - DIV info - Barra de informacao -->
			...
			<?
				$sql = "SELECT * FROM ce_letter WHERE tipo=2 ORDER BY codigo";
			...
			?>					
					...
					?>
						...
						<a href="letter/img/<?=$linha["arq"]?>" target="_blank"><img src="imagens/icon_ver.gif" alt="Visualizar" title="Visualizar" border="0"></a>
						
						<a href="letter_cadastro_img.php?apagar=<?=$linha["tipo"]?>&cod=<?=$linha["codigo"]?>"><img src="imagens/icon_apagar.gif" onclick="javascript: return confirm('Deseja realmente excluir a Newsletter?')" alt="Apagar" border="0"></a></td>
					</tr>
				<?
				}
				?>
