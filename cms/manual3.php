	...
	if($_REQUEST["cadastra"]){
		...
		$telefone = "($ddd) - $tel";
			...
			$ok = 0;
						...
						// Validando Cpf
	if (addslashes(trim($_POST["cpf"])) == "") {
		$ok = 0;
		alert("Preencha o campo CPF corretamente!");
		$cpf = addslashes(trim($_POST["cpf"]));
	} elseif (!valida_cpf(addslashes(trim($_POST["cpf"]))))  {
		$ok = 0;
		alert ("CPF Inválido!");
		$cpf = addslashes(trim($_POST["cpf"]));
	} else {
		...
		if (!$ok) {
			...
			if($_POST["button"]){
			  ...
			  //salvando dados no banco
			    ...
			    if (mysql_query($sql)) {
			    	alert("Aplicação cadastrado com sucesso!");
			    	redireciona("aplica_gerencia.php");
			    }else{
			    	alert("Erro ao cadastrar!");// QUERY
			    }
			}
		}
	}
...
<form id="form1" name="form1" method="post" action="manual3.php?cadastra=1">
  <label></label>
  <label><br />
  </label>
  <table width="450" border="0" cellspacing="2" cellpadding="2" class="label_fonte" style="margin-left: 20px">
    ...
    <tr>
      <td><label>String com respostas:</label></td>
      <td><input name="resp" type="text" id="textfield3" size="140" value="<?=$_POST["resp"]?>" /></td>
    </tr>
 <!--   <tr>
