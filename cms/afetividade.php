...
$seisdez = ($seisdez*100)/$nPessoas;

$zerodois = number_format($zerodois,1,".","");
$media = number_format($media,1,".","");
$cinco = number_format($cinco,1,".","");
$seisdez = number_format($seisdez,1,".","");

$soma = $zerodois+$media+$cinco+$seisdez;
if($soma < 100){
...
// Some data
$data = array($zerodois,$media,$cinco,$seisdez);
...
$item = array();
$desc = mysql_query("SELECT * FROM feedbacks_grupo WHERE item = 'Afetividade'");
...
$p1->SetLegends(array("$zerodois%% ".$item[0],"$media%% ".$item[1],"$cinco%% ".$item[2],"$seisdez%% ".$item[3]));
$graph->legend->SetFont(FF_FONT2,FS_NORMAL,16);
$graph->legend->Pos(0.01,0.01);

$graph->Add($p1);
$graph->Stroke("graficos_grupos/afetividade.png");
?>
