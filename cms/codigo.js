...
function validaForm(){
    ...
    //validar email(verificao de endereco eletrï¿½nico)
...
function Validator(){
    ...
    function mensagensErro(campo){
        ...
        switch (tipo) {
            ...
            case 1: //MIN MAX
                // var min = parseInt(campo.getAttribute("validar_min"));
                // var max = parseInt(campo.getAttribute("validar_max"));
                // mensagem += "\nO campo " + campo.getAttribute("validar") + " deve ter no máximo " + max;
                break;
            case 2: //MAX
                break;
            case 3: //CAMPOS IGUAIS
            ...
            case 4: //CEP
                break;
            case 5: //CAMPO OBRIGATORIO
                mensagem += "\nO campo " + campo.getAttribute("realName") + " é obrigório";
                break;
            case 6: //NUMERO
            mensagem += "\nO campo " + campo.getAttribute("realName") + " deve conter somente números";
                break;
            case 7: //SELECT
            mensagem += "\nVocê deve selecionar uma opção no campo "+ campo.getAttribute("realName");
                break;
            case 8: //DATA
                mensagem += "\nA " + campo.getAttribute("realName") + " é inválida";
                break;
        }
    ...
    this.percorrer = function(formulario){
        ...
        if (ok == 1) 
            ...
            alert("O formulário possui os seguintes erros:" + mensagem);
            return false
        }
...
function busca_cargos(tipo,id){
 	
	if(id != 0){
		if(tipo == 1){
			
			document.cadastra.id_cargo.length=0;
			document.cadastra.id_cargo.disabled = true;
			document.cadastra.id_cargo.options[0] = new Option("Carregando...", 0, false, false);
		}
			
			imput_xml("mn",'busca_cargo.php?tipo='+tipo+'&id='+id,'combos');
	}
}

function combos(texto,id){//você pode tornar essa função dinamica(usando o id) pois ela esta fixa ao getElement("mn")
	//alert(texto);
	if(texto != "erro"){
	       eval(texto);
	}else{
...
function combo(texto,id){//você pode tornar essa função dinamica(usando o id) pois ela esta fixa ao getElement("mn")
	alert(texto);
}
