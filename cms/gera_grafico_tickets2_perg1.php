    ...
    $unidades = explode("|",$alternativas_unidade);
     ...
     if($_GET["n"] == '1') { 
         if($i <= 35) { //Diretoria Técnica Assistencial 
             ...
             $data_test .= total_resposta($id_pesq,$id_agrupa,$i)."|"; 
             $alt_test .= $unidades[$i]."|"; 
         } 
     } 
     ...
     if($_GET["n"] == '4') { 
             if($i <= 99 and $i >= 98) { //Diretoria de Projetos 
                 $a1_resp += total_resposta($id_pesq,$id_agrupa,$i); 
                 $data_test .= total_resposta($id_pesq,$id_agrupa,$i)."|"; 
                 $alt_test .= $unidades[$i]."|"; 
             } 
     ...
     if($_GET["n"] == '6') { 
             if($i >= 100 and $i <= 101) { //Superintendencia
                 $a1_resp += total_resposta($id_pesq,$id_agrupa,$i); 
                 $data_test .= total_resposta($id_pesq,$id_agrupa,$i)."|"; 
                 $alt_test .= $unidades[$i]."|"; 
             } 
     ...
     if($_GET["n"] == '7') { 
             if($i >= 102 and $i <= 104) { //Controladoria 
                 $a1_resp += total_resposta($id_pesq,$id_agrupa,$i); 
                 $data_test .= total_resposta($id_pesq,$id_agrupa,$i)."|"; 
                 $alt_test .= $unidades[$i]."|"; 
             } 
...
$data2 = explode("|",$data_test);
$alternativas2 = explode("|",$alt_test);

for($k = 0; $k < sizeof($alternativas2)-1; $k++)
{
    $alters[$k] = $alternativas2[$k];
}

for($j = 0; $j < sizeof($alternativas2)-1; $j++)
{
    $dataem[$j] = $data2[$j];
}


$graph = new PieGraph(1300,800);
